...
nk_sdl_clear(SDL_Renderer* renderer, struct nk_color col)
{
    ...
    nk_sdl_fill_rect(renderer, rect.x, rect.y, rect.w, rect.h, 0, col);
}
...
nk_sdl_render(struct nk_color clear)
{
    ...
    nk_foreach(cmd, &sdl.ctx)
    {
        ...
        case NK_COMMAND_SCISSOR: {
            ...
            nk_sdl_scissor(renderer, s->x, s->y, s->w, s->h);
        } break;
        case NK_COMMAND_LINE: {
            const struct nk_command_line *l = (const struct nk_command_line *)cmd;
            nk_sdl_stroke_line(renderer, l->begin.x, l->begin.y, l->end.x,
                l->end.y, l->line_thickness, l->color);
        } break;
        case NK_COMMAND_RECT: {
            const struct nk_command_rect *r = (const struct nk_command_rect *)cmd;
            nk_sdl_stroke_rect(renderer, r->x, r->y, r->w, r->h,
                (unsigned short)r->rounding, r->line_thickness, r->color);
        } break;
        case NK_COMMAND_RECT_FILLED: {
            const struct nk_command_rect_filled *r = (const struct nk_command_rect_filled *)cmd;
            nk_sdl_fill_rect(renderer, r->x, r->y, r->w, r->h,
                (unsigned short)r->rounding, r->color);
        } break;
        case NK_COMMAND_CIRCLE: {
            const struct nk_command_circle *c = (const struct nk_command_circle *)cmd;
            nk_sdl_stroke_circle(renderer, c->x, c->y, c->w, c->h, c->line_thickness, c->color);
        } break;
        case NK_COMMAND_CIRCLE_FILLED: {
            const struct nk_command_circle_filled *c = (const struct nk_command_circle_filled *)cmd;
            nk_sdl_fill_circle(renderer, c->x, c->y, c->w, c->h, c->color);
        } break;
        case NK_COMMAND_TRIANGLE: {
            const struct nk_command_triangle*t = (const struct nk_command_triangle*)cmd;
            nk_sdl_stroke_triangle(renderer, t->a.x, t->a.y, t->b.x, t->b.y,
                t->c.x, t->c.y, t->line_thickness, t->color);
        } break;
        case NK_COMMAND_TRIANGLE_FILLED: {
            const struct nk_command_triangle_filled *t = (const struct nk_command_triangle_filled *)cmd;
            nk_sdl_fill_triangle(renderer, t->a.x, t->a.y, t->b.x, t->b.y, t->c.x, t->c.y, t->color);
        } break;
        case NK_COMMAND_POLYGON: {
            const struct nk_command_polygon *p =(const struct nk_command_polygon*)cmd;
            nk_sdl_stroke_polygon(renderer, p->points, p->point_count, p->line_thickness,p->color);
        } break;
        case NK_COMMAND_POLYGON_FILLED: {
            const struct nk_command_polygon_filled *p = (const struct nk_command_polygon_filled *)cmd;
            nk_sdl_fill_polygon(renderer, p->points, p->point_count, p->color);
        } break;
        case NK_COMMAND_POLYLINE: {
            const struct nk_command_polyline *p = (const struct nk_command_polyline *)cmd;
            nk_sdl_stroke_polyline(renderer, p->points, p->point_count, p->line_thickness, p->color);
        } break;
        case NK_COMMAND_TEXT: {
            const struct nk_command_text *t = (const struct nk_command_text*)cmd;
            nk_sdl_draw_text(renderer, t->x, t->y, t->w, t->h,
...
nk_sdl_handle_event(SDL_Event *evt)
{
    ...
    if (evt->type == SDL_WINDOWEVENT_RESIZED) {
        ...
        if (sym == SDLK_RSHIFT || sym == SDLK_LSHIFT) nk_input_key(ctx, NK_KEY_SHIFT, down);
            ...
            if (sym == SDLK_c && state == KMOD_LCTRL)
                nk_input_key(ctx, NK_KEY_COPY, down);
            else if (sym == SDLK_v && state == KMOD_LCTRL)
                nk_input_key(ctx, NK_KEY_PASTE, down);
            else if (sym == SDLK_x && state == KMOD_LCTRL)
                nk_input_key(ctx, NK_KEY_CUT, down);
            else if (sym == SDLK_z && state == KMOD_LCTRL)
                nk_input_key(ctx, NK_KEY_TEXT_UNDO, down);
            else if (sym == SDLK_r && state == KMOD_LCTRL)
                nk_input_key(ctx, NK_KEY_TEXT_REDO, down);
            else if (sym == SDLK_LEFT && state == KMOD_LCTRL)
                nk_input_key(ctx, NK_KEY_TEXT_WORD_LEFT, down);
            else if (sym == SDLK_RIGHT && state == KMOD_LCTRL)
                nk_input_key(ctx, NK_KEY_TEXT_WORD_RIGHT, down);
            else if (sym == SDLK_b && state == KMOD_LCTRL)
                nk_input_key(ctx, NK_KEY_TEXT_LINE_START, down);
            else if (sym == SDLK_e && state == KMOD_LCTRL)
